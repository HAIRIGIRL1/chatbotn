...
export class XDatabase {
    ...
    async #updateMONGO() {
        ...
        for (const thread of threads) {
            ignoreList.forEach((e) => delete thread[e]);
        ...
        for (const user of users) {
            ignoreList.forEach((e) => delete user[e]);
        ...
        for (const effect of effects.values()) {
            ignoreList.forEach((e) => delete effect[e]);
        ...
        logger.custom(`FINISHED UPDATING: ${Date.now() - start}ms`, "MONGO-DB");
    }
    ...
    async #handleMongoDB() {
        ...
        for (const effect of effectsData) {
            if (!effects.has(effect.pluginName)) {
                ...
                for (const eD of effect.exp) {
                    for (const efD of eD.effects) {
                        pluginEffects.exp.add(eD.userID, efD.name, efD.value);
                    }
                }

                for (const eD of effect.money) {
                    for (const efD of eD.effects) {
                        pluginEffects.money.add(eD.userID, efD.name, efD.value);
                    }
                }
